...
## Tech Stack
- **로컬 인프라**: Docker Compose (backend 컨테이너) + Railway PostgreSQL
...
```
pocketman-dev-su/
├── docker-compose.yml          # Backend 컨테이너 실행 (DB는 Railway 사용)
├── backend/
...
| `ALLOWED_ORIGINS` | Backend | CORS 허용 도메인 목록(콤마 구분) |
| `NEXT_PUBLIC_API_BASE_URL` | Frontend | Backend 엔드포인트 URL |
...
5. Docker Compose는 `backend`만 기동하고 DB는 Railway를 사용한다.
6. 배포/롤백 절차를 문서화한다.
7. `scripts/shared/feature_mapping.py`와 `scripts/user_poc/extractor.py`는 Backend Docker 이미지에 포함되어야 한다 (`cv_adapter.py` 의존).

## 로컬 기동 절차
```bash
# 1. Railway DB 연결 문자열 설정 (.env)
cp .env.example .env
# DATABASE_URL=postgresql://<user>:<password>@<tcp-proxy-host>:<port>/railway

# 2. Railway DB에 스키마 적용
