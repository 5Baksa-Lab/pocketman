...
type Status = "checking" | "ready" | "redirecting" | "login-gate";
...
export default function PlazaPage() {
  ...
  if (status === "login-gate") {
    return (
      <div className="flex min-h-[calc(100vh-72px)] items-center justify-center">
        <Modal open onClose={() => router.push("/")}>
          <div className="p-6 max-w-sm text-center">
            <p className="text-4xl mb-4">🏞️</p>
            <h3 className="text-lg font-black text-ink mb-2">광장에 입장하려면 로그인이 필요해요</h3>
            <p className="text-sm text-ink/60 mb-6">
              로그인 후 나만의 크리처로 광장을 탐험할 수 있어요!
            </p>
            <div className="flex gap-3">
              <Button variant="ghost" className="flex-1" onClick={() => router.push("/")}>
                홈으로
              </Button>
              <Button className="flex-1" onClick={() => router.push("/login?next=/plaza")}>
                로그인
              </Button>
            </div>
          </div>
